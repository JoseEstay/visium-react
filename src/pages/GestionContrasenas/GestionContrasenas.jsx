...
export default function GestionContrasenas() {
  ...
  const openChange = (target) => { setSelected(target); setPassword(""); setConfirmation(""); setMessage(""); };
  const submit = (event) => {
  ...
  return <section className="password-page">
    ...
    <div className="password-grid">
      ...
      <div className="password-card">{selected ? <form onSubmit={submit}><h2>Cambiar contraseña</h2><p>Usuario: <strong>{selected.nombre}</strong></p><label>Nueva contraseña<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength="6" required /></label><label>Confirmar contraseña<input type="password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} minLength="6" required /></label>{message && <p className={message.includes("correctamente") ? "success" : "error"}>{message}</p>}<button type="submit">Guardar contraseña</button></form> : <div className="empty-password"><i className="bi bi-shield-lock" /><p>Selecciona un usuario para cambiar su contraseña.</p></div>}</div>
    </div>
  </section>;
}
