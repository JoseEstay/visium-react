...
export default function GestionAdministrativa() {
  ...
  return <section className="admin-page">
    ...
    {form && <div className="admin-modal" onClick={() => setForm(null)}><form onSubmit={save} onClick={(event) => event.stopPropagation()}><div className="modal-title"><h2>{records.some((record) => record.id === form.id) ? "Modificar" : "Agregar"} {config.title.slice(0, -1)}</h2><button type="button" onClick={() => setForm(null)}>&times;</button></div>{formFields.map((field) => <label key={field}>{label(field)}{field === "usuarioPassword" ? <span className="password-field"><input required type={showUserPassword ? "text" : "password"} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} /><button type="button" onClick={() => setShowUserPassword((visible) => !visible)} aria-label={showUserPassword ? "Ocultar contraseña" : "Mostrar contraseña"}><i className={`bi bi-eye${showUserPassword ? "-slash" : ""}`} /></button></span> : <input required disabled={resource === "recepcionistas" && user.rol === "administrador sucursal" && (field === "sucursalId" || field === "sucursal")} value={form[field]} onChange={(event) => setForm({ ...form, [field]: event.target.value })} />}</label>)}<button className="save" type="submit">Guardar</button></form></div>}
